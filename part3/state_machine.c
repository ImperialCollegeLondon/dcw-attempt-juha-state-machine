...
void state_machine( state_t statename )
{
    state_id_t stateno = lookup_state( statename );
	...
	action_id_t event = event_id_seq[e];
	assert( event >= 0 && event < num_actions );

	state_id_t newstate = transition_tbl[stateno].newstate[event];
	transition_function transition = transition_tbl[stateno].transition[event];

	printf( "debug: in state %s, event %s, newstate %s\n",
		statenames[stateno], actionnames[event], statenames[newstate] );

        if( newstate != STATE_NOCHANGE && newstate != stateno )
	{
                /* true state change */
		#if 0
		exit_state_function ext = state_tbl[stateno].on_exit;
                if( ext != NULL )
		{
                    ext();
                }
		...
		#if 0
		enter_state_function ent = state_tbl[stateno].on_entry;
                if( ent != NULL )
		{
                    ent();
                }
